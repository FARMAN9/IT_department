...
function Timetable() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await axios.post("http://localhost:4000/api/uploadTimeTable", formData, {
        headers: {
      ...
      await dispatch(fetchTimeTableData());
      toast.success("TimeTable uploaded successfully!");
      setShowUploadModal(false);
      setUploadForm({ programme: "", batch: "", semester: "", file: null });
  ...
  const handleUpdate = async () => {
    ...
    try {
      const loadingToast = toast.loading("Updating TimeTable...");
      await axios.put(
        `http://localhost:4000/api/uploadTimeTable/${selectedTimeTable._id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      setEditForm({ programme: "", batch: "", semester: "", file: null });
      toast.dismiss(loadingToast);
    } catch (err) {
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:4000/api/deleteTimeTable/${selectedTimeTable._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const totalPages = Math.ceil(filteredTimetable.length / rowsPerPage);
  const currentRows = filteredTimetable.slice(indexOfFirstRow, indexOfLastRow);
  ...
  return (
    <>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex z-50 items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h3 className="text-lg font-bold mb-4">Upload New TimeTable</h3>
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex z-50 items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h3 className="text-lg font-bold mb-4">Edit TimeTable</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">
                  Programme
                ...
                <label className="block text-sm font-medium mb-1">
                  ...
                  <div className="text-red-500 border border-gray-300 p-2 flex rounded-md w-full flex-wrap overflow-hidden">
                    <p className="text-xs w-full">
                      <a href={selectedTimeTable.TimeTable}>
                        <FaFilePdf size={20} /> {selectedTimeTable.TimeTable}
                      </a>
                    </p>
                  </div>
                </label>
              ...
              <div className="space-y-4 p-4">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4">
                  <div className="text-sm text-gray-600">
                    ...
                    {Math.min(indexOfLastRow, filteredTimetable.length)} of{" "}
