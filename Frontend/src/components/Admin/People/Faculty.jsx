...
import { fetchFacultyData } from "../../../Features/UserSlice";
import {
  HiChevronLeft,
  HiChevronRight,
  HiSearch,
...
function Faculty() {
  ...
  const handleUpload = async () => {
    ...
    formData.append("designation", uploadForm.designation);
    formData.append("image", uploadForm.file );

    try {
      const loadingToast = toast.loading("Adding faculty member...");
      await axios.post(
        "http://localhost:4000/api/signUpForUsers",
        formData,
        {
          headers: {
      ...
      toast.success("Faculty member successfully added!");
  ...
  const handleRemoveImage = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:4000/api/removeFacultyImage/${selectedArea._id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  if (loading) return  (<MainCard title="Faculty"> <Loading /> </MainCard>) ;
  if (error) return  (<MainCard title="Faculty"> <Errors error={error} /> </MainCard>) ;
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex z-50 items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h3 className="text-lg font-bold mb-4">Add New Faculty Member</h3>
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <input
                  type="text"
                  className="w-full p-2 border rounded-md"
                  value={uploadForm.designation}
                  onChange={(e) =>
                    setUploadForm({ ...uploadForm, designation: e.target.value })
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setUploadForm({ ...uploadForm, password: e.target.value })
                  }
                />
              </div>
              <div>
                          ...
                          <td className="py-3 px-4">
                            {item.image ? (
                              <img
                                src={item.image}
                                alt={item.name}
                                className="w-24 h-24 rounded-full object-fit border-2 border-teal-600 border-dashed break-all "
                              />
                            ) : (
                              <div className="w-24 h-24 rounded-full bg-gray-200 flex items-center justify-center">
                                <span className="text-gray-500 text-2xl font-bold break-all capitalize">{item.name.charAt(0)}</span>
                              </div>
                            )}
                          </td>
